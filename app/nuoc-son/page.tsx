...
export default function PaintProductsPage() {
    ...
    const loadMore = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/products?page=${page}&limit=${ITEMS_PER_PAGE}&type=PAINT`);
            const data = await res.json();

            setProducts((prev) => {
                const newProducts = data.products.filter((p: Product) => !prev.some((existing) => existing.id === p.id));
    ...
    const tabs = [
        ...
        { name: `Nước sơn (${totalCount || products.length})`, href: '/nuoc-son', active: true },
        { name: 'Gạch ốp lát', href: '/gach', active: false },
    ];
                    ...
                    <div className="mx-auto max-w-7xl px-4 py-8 sm:py-10 sm:px-6 lg:px-8">
                        <AnimatedSection>
                            ...
                            <p className="mt-2 text-gray-600">
                                Khám phá bộ sưu tập sơn chất lượng cao
                            </p>
                        </AnimatedSection>
                    </div>
                </div>
